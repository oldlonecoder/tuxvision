...
std::string  alu::mNil = "";
#define debug_out ;
//book::debug() << book::fn::fun << "rhs=" << rhs << ", this = " << v.d << " : types: " << lexer_component::type_name(T);
...
alu alu::operator-(const alu& rhs)
{
    //book::debug() << book::fn::fun << " rhs=" << rhs.to_str();
    lrtext(rhs)
    {
...
alu& alu::operator*=(const alu& rhs)
{
    ...
    lr_number(rhs)
    {
        book::debug()
            << color::orange5 << "alu"
            << color::white << "::"
            << color::white << book::fn::endl << ":" << book::fn::endl << "{" << book::fn::endl
        ...
        v.d *= rhs.v.d;
        book::out() << color::white << " = " << color::yellow << v.d  << color::white <<  book::fn::endl << '}' <<  book::fn::endl ;
        return *this;
    }
...
{
    book::debug() << book::fn::fun << color::yellow << (*this)() <<color::white << ":"  ;
    lr_number(*this)
    {
        if(v.d < 0)
        {
        ...
        for (int j = 1; j <= N; j++)
            fac *= j;
        book::debug() << book::fn::fun << color::yellow << fac <<color::white << ":"  ;
        return { fac };
    }
    throw book::exception()[ book::except() <<"cannot execute factorial operator on " << lexer_component::type_name(T)];
    return { false };
